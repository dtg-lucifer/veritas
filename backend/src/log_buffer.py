...
class RealTimeLogBuffer:
    def __init__(self):
        ...
        # user -> historical baseline counters
        self.user_historical_mean: Dict[str, Dict[str, float]] = defaultdict(lambda: defaultdict(float))
    ...
    def ingest_event(self, event: Dict[str, Any]) -> Tuple[str, str, Dict[str, float]]:
        ...
        # 2. File copy events
            ...
            if ext in [".doc", ".docx", ".pdf", ".txt", ".xls", ".xlsx", ".csv", ".ppt"] or filename.endswith((".doc", ".docx", ".pdf", ".txt")):
                state["file_doc_pdf_count"] += 1
            elif ext in [".exe", ".zip", ".rar", ".7z", ".bin", ".py"] or filename.endswith((".exe", ".zip", ".bin")):
                state["file_zip_exe_count"] += 1
            if after_h:
                state["file_after_hours"] += 1
                state["file_surge_zscore"] += 3.0
        ...
        # 3. Email events
            ...
            if "@dtaa.com" not in to_addr and "@" in to_addr:
            ...
            if size > 10_000_000:
                state["email_bytes_surge_zscore"] += 4.0
        ...
        state["sensitive_web_count"] = (
            state["http_wikileaks_count"] + state["http_job_search_count"] +
            state["http_cloud_storage_count"] + state["http_hacking_count"]
        )
